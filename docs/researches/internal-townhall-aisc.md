...
### 1.2 "Department Journey" Process Flow
...
**Pattern:** 3-panel narrative repeated across all project slides
- **Panels:**
  1. PROBLEM STATEMENT (2-3 challenges with icons)
  2. SOLUTION (3-5 key capabilities with colored accents)
...
## 2. AISC FRAMING PATTERNS: How Nanovest Presents AI Organization-Wide

### 2.1 "Ready to Scale — Aligned to Win" Brand Positioning
**Visual/Verbal:** Tagline at top of presentation
- **Signals:** Ambition (Ready to Scale) + Unity (Aligned) + Victory mindset (to Win)
- **Tone:** Confident without arrogance, forward-looking
- **Context:** Works for fintech/crypto (Nanovest); would need localization for mining (e.g., "Ready to Optimize — Aligned to Protect")

### 2.2 AI as Business Multiplier, Not Cost Center
**Narrative arc across slides:**
- Slide 1: Showcase "10+ Projects" + "50+ Session Attendance" → Proof of adoption momentum
...
- "Anyone can build" (Anyone Can Build A Smart Dashboard session)
- "No engineering needed" (Web3 Price Alert built by Product team with n8n)
...
- 8+ AI Champions (embedded per department)
- Biweekly NIC Friday sessions (cross-dept knowledge sharing)
...
1. **Phase 1 (Current):** Internal automation (chatbots, alerts, doc generation) → prove ROI internally
...
| 3 | Naura — AI Chatbot | Customer support automation via RAG + intent detection + Zendesk escalation. Problem → Solution → How It Works structure. Metrics: 14k+ conversations, 75% deflection, +10% monthly growth | Topic 3: Process Improvement (RPA/IPA) + Topic 5: AI Techniques (RAG) + Topic 6: Tools Ecosystem | HIGHLY REUSABLE | **Template reuse:** Problem-Solution-How It Works 3-panel layout is **foundational**. Substitute customer support with "Safety incident reports" or "Equipment maintenance requests." Same flow: intake → AI understanding → knowledge base → escalation. Show Naura UI screenshots → adapt with mine-safety or equipment dashboards. |
...
## 4. DESIGN & ANIMATION COMPONENTS FROM REPOSITORY

### 4.1 Visual Design System (Token-Based)
**File:** `/Users/macbook/Projects/_web_presentation/nanovest-townhall-presentation/src/tokens.js`
...
- **Titles:** Trebuchet MS 28pt bold (consistent across all slides)
- **Subtitles:** Calibri 11pt, allows colored keyword highlights
- **Body/cards:** Calibri 9.5-10.5pt (dense but readable on 16:9 slides)
- **KPI values:** Calibri 28-36pt bold (impacts visual hierarchy)
- **Section headers:** Trebuchet MS 11pt with 1.5pt char spacing

**Slide Dimensions:** 10" × 5.625" (16:9, standard widescreen)

### 4.2 Reusable Slide Layouts

#### Layout A: Milestones + Solutions Grid (Slide 1)
**File:** `/Users/macbook/Projects/_web_presentation/nanovest-townhall-presentation/src/slides/slide1-milestones.js`

**Components:**
- **Left column (38%):** Vertical timeline with circle nodes + event cards (date, title, summary, speaker)
  - Timeline line: thin rectangle (0.02" wide) with circle nodes (0.07" radius)
  - Event spacing: 0.64" between items, accommodates 4 events
  - Colors: Pink accent for timeline, cyan/orange for speaker
- **Right column (62%):** Solutions list with colored accent bars
  - Each row: 0.36" tall, subtle dark background (rounded 0.04" corners)
  - Colored vertical line (left edge, 0.035" wide) matches solution category
  - Row layout: icon → solution name → status badge → category badge
  - Badges: 0.52" (status) + 0.92" (category), 0.16" tall, rounded 0.03", semi-transparent fill
- **Bottom: 4 KPI cards (2.20" × 0.58" each)
  - Accent line at top (0.03" tall)
...
**Components:**
- **Left side (38%):** 2 images side-by-side (portrait orientation, aspect-ratio preserved)
  - Image heights computed to match right-column content height
  - Gap between images: 0.06"
  - Images scale dynamically based on aspect ratio
- **Right side (62%):** 3 sections stacked
  1. **Problem Statement** (0.14" title + 0.015" gaps between rows, 2-3 problem rows @ 0.34" each)
     - Icon + problem title + description
     - Colored accent icon on left
  2. **Solution** (same row structure, 3-5 rows)
     - Icon + capability title + description
     - Colored highlights for key terms
  3. **How It Works** (0.32" diagram box + 0.14" tech stack line)
     - Process flow: 5-6 sequential boxes with arrows
     - Each box color-coded to solution component
...
**Components:**
- **Left side: Pillar grid**
  - 4 pillars stacked (Knowledge Programs, AI Tools, Technical Consultation, Measurement)
  - Each pillar: 0.22" colored left border + icon + title + 2-3 bullet points
  - Icons in semi-transparent colored circles (0.26" diameter @ 80% transparency)
- **Right side: Dual-track descriptions**
  - Personal Enablement vs. Team Enablement (2-column comparison)
  - Each track has 2-3 key attributes
- **Bottom: Department AI Journey (5-step flow)**
  - Boxes: Learn (cyan) → Experiment (orange) → Build (orange) → Integrate (purple) → Own (green)
  - Each box: 0.16" title + small description + connected by arrows
...
**Pattern:** FontAwesome React icons rendered as base64 PNGs, color-mapped per use case
- **Examples:**
  - `FaCalendarAlt` (Pink) → scheduling/events
  - `FaRobot` (Cyan) → AI/automation
  - `FaBell` (Red/Orange) → alerts/monitoring
  - `FaChartLine` (Orange/Cyan variants) → analytics
  - `FaRss` (Purple/Orange variants) → data sources
  - `FaBitcoin` (Yellow) → crypto/assets
  - `FaFileContract` (Purple/Cyan variants) → documents

**Best practice:** Each icon inherits 1-2 color variants based on context (same icon reused, color changed per row/section).

**Application:** Create mining-specific icons or reuse FontAwesome:
  - `FaHardHat` → safety
  - `FaCog` → equipment/maintenance
  - `FaExclamationTriangle` (Red) → risk/anomaly
  - `FaHeartbeat` → operational health
  - `FaWrench` → maintenance/repair
...
- `tokens.js` → centralized design system (edit once, apply everywhere)
- `slides/*.js` → modular slide builders (each slide = self-contained .js file)
- `icons.js` → icon preloading and color mapping

### 4.5 Animation & Motion Patterns
**Note:** PDF doesn't show animations, but repository structure enables:
- **Timeline visualization** (Slide 1 left): events animated in sequentially from left to right
- **Badge reveals** (Slide 1 right): solution rows appear with staggered animation (top to bottom)
- **Process flow diagrams** (Slide 2-6 right bottom): boxes illuminate step-by-step as narration progresses
...
| **3. Process Improvement (BPA/RPA/IPA)** | Slide 3 (3rd party alerts automation), Slide 4 (stocks sentiment multi-stage pipeline), Slide 7 (legal docs generation, Web3 alerts, HR/Finance/Ops use cases) | Concrete RPA examples (n8n, scheduled tasks, API orchestration); IPA examples (AI decides escalation/approval); BPA examples (workflow redesign around AI capabilities) | 30-40 min, hands-on lab demo |
| **4. AI Engineering (Prompt/Context/Harness)** | Slide 6 (Cinematic Ads multi-agent system shows prompt engineering abstraction) | How to build AI features: simple UI hides complex prompts; multi-agent patterns; context injection (knowledge bases for RAG) | 25-30 min, lab focus |
| **5. AI Techniques (MCP/RAG/Skills/Orchestration)** | Slide 2 (RAG for Naura), Slide 3-5 (API orchestration for alerts/sentiment), Slide 6 (multi-agent for video), Slide 9 (NotebookLM/BigQuery MCPs) | RAG pattern: problem (unreliable manual processes) → solution (dynamic knowledge retrieval); MCP examples (data connectors, external APIs); Skills (chained agents for complex workflows) | 35-40 min, lab deep-dive |
...
## 6. SOURCE CITATIONS & DOCUMENT REFERENCES

### Primary Sources

1. **Nanovest Townhall AISC PDF (Q1 2026)**
   - Path: `/Users/macbook/Projects/_web_presentation/berau-presentation/docs/references/nanovest-townhall-aisc.pdf`
   - 10 slides, full-bleed background imagery, animated HTML render
   - Pages analyzed: All (1-10)
   - Key dates: Sep 2025 (AISC start) through Q1 2026 (townhall presentation)

2. **Nanovest Townhall Presentation Repository**
   - Path: `/Users/macbook/Projects/_web_presentation/nanovest-townhall-presentation/`
   - Technology: pptxgenjs (Node.js library for PPTX generation)
   - Architecture: Modular slides, token-based design system, icon preloading
   - Key files referenced:
     - `/src/tokens.js` — Color palette, typography, layout constants
     - `/src/helpers.js` — Reusable component builders (KPI cards, section headers, card panels, image fitting)
     - `/src/generate.js` — Icon system and slide generation orchestration
     - `/src/slides/slide1-milestones.js` — Timeline + solutions grid layout
     - `/src/slides/slide2-naura.js` — 3-panel project showcase template (reused for slides 3-6)
     - `/src/slides/slide7-roadmap.js` — 2-track enablement + department journey flow

### Referenced Nanovest Projects & Capabilities
- **Naura** — Customer support AI chatbot (RAG + intent detection + Zendesk escalation)
- **3rd Parties Announcement Alert** — Orchestrated monitoring (RSS → deduplication → classification → alerting)
- **Stocks Hot Now** — Multi-agent sentiment pipeline (filter relevance → analyze sentiment → rank → summarize)
- **Cinematic Ads** — Multi-agent video generation (creative director abstraction hiding prompt engineering)
- **Legal Docs Generation** — AI-powered document automation (Gemini Pro + approval workflow)
- **Web3 Coins Price Alert** — No-code automation (Product team built with n8n, BirdEye, MoEngage)
- **15+ additional projects** across HR, Finance, Operations (referenced in Slide 7)

### Key Quotes & Frameworks Extracted
- **Brand tagline:** "Ready to Scale—Aligned to Win"
- **Core narrative:** "Building AI DNA across Nanovest — from engineering-led workshops to company-wide adoption"
- **Department journey:** Learn → Experiment → Build → Integrate → Own (5-stage maturity model)
- **Adoption target:** 80%+ adoption
...
6. **KPI card design** → Use for mining metrics (incident prevention %, equipment uptime %, shift safety scores)
7. **Color palette** → Rebrand to mining (Red = safety/high-risk, Green = operational OK, Orange = caution, Cyan = data/monitoring)
8. **Icon system** → Add mining-specific icons (hard hat, pit diagram, equipment, safety vest)

### Lower-Priority (Post-Workshop)
9. **Roadmap/Future Works** (Slide 9) → Use for "Berau AI 2026-2027 Vision" (not in initial workshop, but for management buy-in deck)
10. **"Other Showcases" breadth** (Slide 7) → Inform "What's Next" breakout sessions (HR safety certification AI, supply chain optimization, compliance automation)
...
|---------|------|---------|
| 1.0 | May 6, 2026 | Initial mining report. All 10 slides analyzed, 4 key layout patterns extracted, 5 top-reusable elements identified, full content mapping created. |

---

**End of Report**

Generated for Adri (Nanovest AI Workshop Lead)  
Distribution: Berau Coal Energy AI Workshop Planning Committee  
Confidentiality: Internal Use Only — References Nanovest proprietary presentation
