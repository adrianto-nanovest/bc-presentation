...
import {
  E9ContextTheWall,
  e9Slide,
} from "@/slides/foundation-core-section-e/e9-context-the-wall";
...
test("E.8 declares 2 steps with canonicalPose=1", () => {
  expect(e9Slide.steps).toBe(2);
  expect(e9Slide.canonicalPose).toBe(1);
  expect(e9Slide.animationMode).toBe("step-reveal");
  expect(e9Slide.sectionKey).toBe("fundamentals");
...
test("E.8 renders the FIG label `FIG. E.8 · CONTEXT · PITFALLS`", () => {
  ...
  expect(fig?.textContent).toMatch(/FIG\.\s*E\.9.*CONTEXT.*PITFALLS/i);
});
...
test("step 0 → headline + 4 pitfall cards visible; footer hidden; canvas empty", () => {
  ...
  for (const p of e9Content.pitfalls) {
  ...
  expect(screen.getByTestId("e9-footer").className).not.toMatch(/\bon\b/);
...
test("step 1 → footer reveals; cards stay visible", () => {
  ...
  const footer = screen.getByTestId("e9-footer");
