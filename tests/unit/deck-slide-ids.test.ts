...
/** 72 files under `src/slides/` plus `hexLadderDevSlide` in the registry — gh#34's
 ...
 *  `invest-own-proof` (gh#56, the third) and `invest-chicken-egg` (gh#57, the
 *  fourth). A migration sentinel, not the completeness proof — see the note on
 *  `findSlideDefs`; the scan is proven complete against the live decks in
 *  `deck-composed-numbering.test.ts`. */
const EXPECTED_DEFS = 73;
