...
/** 84 files under `src/slides/` plus `hexLadderDevSlide` in the registry — gh#34's
 ...
 *  and closing it at §6.8's three), `invest-base-rates` (gh#70, the fifteenth —
 *  the SECOND to land at the head of an existing run, and the one that closes
 *  `invest` at §6.7's five from its front), and `shape-tam-kotter` (gh#71, the
 *  SIXTEENTH — a MID-RUN insert between `f8-your-agentic-os` and `shape-middle-out`,
 *  and the one that closes `shape` at §4.3's four and the leader deck itself at 73
 *  rows). A migration
 *  sentinel, not the
 ...
 *
 *  A FILE IS COUNTED HERE AND A FIGURE IS NOT, and the four tickets before this line
 ...
 *  FOUR composed figures, gh#71 added ONE file and moved exactly ONE — and this number
 *  went up by exactly one on each of the four.
