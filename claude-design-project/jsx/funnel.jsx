/* global window, React */
// Funnel animation — copper ladder. Particles flow left→right through 4 rings.
// Each ring glows ONLY when its corresponding card is hovered (hoveredIndex === ring index).
// Ring centers are tuned to align with a 4-column card grid sitting below this canvas
// (cards laid out with paddingLeft/Right = 10% and 4 equal columns + 0 gap).
function FunnelAnimation({ hoveredIndex }) {
  const canvasRef = React.useRef(null);
  const stageRef = React.useRef(null);
  const hoverRef = React.useRef(hoveredIndex);
  React.useEffect(() => { hoverRef.current = hoveredIndex; }, [hoveredIndex]);
  ...
  React.useEffect(() => {
    ...
    // Copper ladder — left → right matches the cards below:
    // WRITE (copper-700, darkest) → SELECT (600) → COMPRESS (500) → ISOLATE (400, brightest)
    const rings = [
      { color: '#7a4626' }, // copper-700 → WRITE
      { color: '#9c5a30' }, // copper-600 → SELECT
      { color: '#b86e3d' }, // copper-500 → COMPRESS
      { color: '#c98548' }, // copper-400 → ISOLATE
    ];

    // Place ring centers at 0.20, 0.40, 0.60, 0.80 → aligns with the 4 cards
    // below (which use 10% horizontal padding and 4 equal columns).
    const layout = () => {
    ...
    const dots = [];
    for (let i = 0; i < NUM_DOTS; i++) dots.push(makeDot(-0.1 + Math.random() * 1.25));

    function smooth(t) { t = Math.max(0, Math.min(1, t)); return t * t * (3 - 2 * t); }

    function particleY(d) {
      const r0 = rings[0];
    ...
    function justCrossed(prevX, x, ringX) { return prevX < ringX && x >= ringX; }
    function ringInfluence(x, ring) {
    ...
    function drawGridDot(px, py, r, alpha) {
      ctx.beginPath();
      ctx.arc(px, py, r, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(229,229,229,${alpha})`;
      ctx.fill();
    }
    function drawGlowDot(px, py, r, color, intensity) {
    ...
    function drawArrow(x1, y1, x2, y2, opts = {}) {
      ...
      ctx.fillStyle = color;
      ctx.beginPath();
      const h = headLen, w = thickness * 1.6;
      ctx.moveTo(x2, y2);
    ...
    function drawCurvedArrow(p0, p1, p2, opts = {}) {
      const thickness = opts.thickness || 8;
      const color = opts.color || 'rgba(229,229,229,0.85)';
      const headLen = opts.headLen || 10;
    ...
    function drawRing(r, glow) {
      ...
      // Dim non-hovered rings slightly so the highlighted one reads stronger
    ...
    // Eased per-ring glow values, target = 1 if hovered, else 0.
    ...
    function frame(now) {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        d.x += d.speed * dt;
        d.phase += dt;
        let intensity = 0, nearestRingColor = null, nearestGlow = 0;
        for (let k = 0; k < rings.length; k++) {
          const R = rings[k];
...
window.FunnelAnimation = FunnelAnimation;
